...
def spin(p):
	if(p%2==0):
		#Choose states indexed by even number to have spin up
		return 1
	else:
		#Choose states indexed by odd number to have spin down
		return -1

def OBME(Z):

	"""
	Hardcoded setup of the one-body matrix elements
...
def spatial_TBME(Z):
	...
	u 		   = np.zeros((3,3,3,3))
	u[0,0,0,0] = (5*Z)/8.0
	u[0,0,0,1] = (4096.0*np.sqrt(2)*Z)/64827.0
	u[0,0,0,2] = (1269.0*np.sqrt(3)*Z)/50000.0
	u[0,0,1,0] = (4096.0*np.sqrt(2)*Z)/64827.0
	u[0,0,1,1] = (16.0*Z)/729.0
	u[0,0,1,2] = (110592.0*np.sqrt(6)*Z)/24137569.0
	u[0,0,2,0] = (1269.0*np.sqrt(3)*Z)/50000.0
	u[0,0,2,1] = (110592.0*np.sqrt(6)*Z)/24137569.0
	u[0,0,2,2] = (189.0*Z)/32768.0
	u[0,1,0,0] = (4096.0*np.sqrt(2)*Z)/64827.0
	u[0,1,0,1] = (17.0*Z)/81.0
	u[0,1,0,2] = (1555918848.0*np.sqrt(6)*Z)/75429903125.0
	u[0,1,1,0] = (16.0*Z)/729.0
	u[0,1,1,1] = (512.0*np.sqrt(2)*Z)/84375.0
	u[0,1,1,2] = (2160.0*np.sqrt(3)*Z)/823543.0
	u[0,1,2,0] = (110592.0*np.sqrt(6)*Z)/24137569.0
	u[0,1,2,1] = (29943.0*np.sqrt(3)*Z)/13176688.0
	u[0,1,2,2] = (1216512.0*np.sqrt(2)*Z)/815730721.0
	u[0,2,0,0] = (1269.0*np.sqrt(3)*Z)/50000.0
	...
	u[2,2,2,2] = (17.0*Z)/256.0
	return u

def antiSymmetrize(v_spatial):
	
	L   = 2*v_spatial.shape[0]
	vAS = np.zeros((L,L,L,L))
		...
		for q in range(0,L):
			...
			for r in range(0,L):
				...
				for s in range(0,L):
					...
					val1 = 0
					if(sp == sr and sq == ss):
						val1 = v_spatial[p//2,q//2,r//2,s//2]
					val2 = 0
					if(sp == ss and sq == sr):
						val2 = v_spatial[p//2,q//2,s//2,r//2]

					vAS[p,q,r,s] = val1-val2
