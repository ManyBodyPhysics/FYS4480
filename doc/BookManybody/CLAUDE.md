...
chapter14.tex      Optimization and resampling: the energy gradient as a
                   covariance, steepest descent / momentum / quasi-Newton /
                   stochastic reconfiguration, the vectorised production run,
...
chapter16.tex      Six electrons by all five methods: HF, MBPT and CC from
                   chapter 11 set against VMC and DMC with a Slater-Jastrow
                   trial function. Uses sections 2.11-2.14 (Sherman-Morrison,
                   derivative ratios, spin factorisation), spin-dependent cusp
                   coefficients, Metropolis sampling of the spin assignment,
                   the Umrigar drift cutoff, and an omega scan. Written from
                   scratch; benchmarked against Lohne et al., PRB 84, 115302
...
`chapter14.tex`, `diffusionmc.ipynb` to `chapter15.tex` and
`manybodymc.ipynb` to `chapter16.tex`; the older set is
`notation.ipynb`,
...
| `manybodymc.py` | 16 | Slater-Jastrow VMC and DMC for N electrons in a 2D dot, importing the oscillator orbitals from `slater_update.py`: Sherman-Morrison ratios and derivative ratios, spin-dependent cusp coefficients (1 antiparallel, 1/3 parallel in 2D), Metropolis sampling of the spin assignment at fixed Sz, the Umrigar drift cutoff, the chapter-14 optimiser, and DMC with `spin_moves` = equilibration/never/always. Reduces to `vmc.py` exactly at N=2. Runs in about four minutes |
