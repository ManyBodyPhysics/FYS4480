...
week40.do.txt  pmm.tex  dft.do.txt  mbpt.ipynb
...
3. `book.tex` loads `pstricks` and `[dvips]{epsfig}`, though nothing in
   chapters 1–9 uses them. Dropping them would allow a plain `pdflatex` route.
...
- Equations: numbered `equation` environments with labels in the pattern
  ...
  as `Eq.~(\ref{eq:2-1a})`.
- All nine chapters use numbered `\section{...}`. Chapters 1--3 end with an
  `Exercises` section containing warm-up exercises followed by the weekly
  FYS4480 sessions and their answers; chapters 4--9 relegate the long
...
Chapter 9 is deliberately algebraic: the diagrammatic representation of
perturbation theory has not been written yet.
...
`chapter8.tex` and `mbptheory.ipynb` to `chapter9.tex`; the older set is
`notation.ipynb`,
...
Every number quoted in chapters 1–9 comes from one of these; each file runs
