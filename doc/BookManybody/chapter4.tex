...
transformation of Section~\ref{sec:jordanwigner} maps it back onto
Eq.~(\ref{eq:4-generalH}), and the mapping is instructive in its own right.
Section~\ref{sec:qubitmodels} runs the same transformation the other way and
puts all the models of this chapter on a qubit register.
...
\item When $q=r$ the operator is
  ...
  again.  Together with its Hermitian conjugate, which contributes the same
  sum over again, these pieces merely renormalise $g\to g+2f$; they move whole
  pairs and change nothing structurally.
...
picks up the promised $g\to g+2f$.
...
%----------------------------------------------------------------------
\section{The quantum Heisenberg model}
\label{sec:heisenberg}

\index{Heisenberg model}
The strong-coupling limit of the previous section produced a model of
localised spins, and that model deserves a section of its own.  At half
filling and $U\gg t$ there is no room for an electron to hop, the charge
degrees of freedom are frozen out, and all that is left on each site is a spin
one-half.  What remains is the \emph{quantum Heisenberg model},
\begin{equation}
  \boxed{\;
  \hat{H} = J\sum_{\langle ij\rangle}\bm{S}_i\cdot\bm{S}_j
    - h\sum_i \hat{S}^z_i \;}
  \label{eq:4-heisenbergH}
\end{equation}
with $\bm{S}_i=\tfrac12\bm{\sigma}_i$ built from the Pauli matrices of
Section~\ref{sec:pauli}, and $h$ an external field which we shall usually set
to zero.  For $J>0$ neighbouring spins prefer to be antiparallel and the model
is antiferromagnetic; for $J<0$ it is ferromagnetic.

Written out with $\hat{S}^{\pm}=\hat{S}^x\pm i\hat{S}^y$ the exchange term is
\begin{equation}
  \bm{S}_i\cdot\bm{S}_j
   = \hat{S}^z_i\hat{S}^z_j
     + \frac12\left(\hat{S}^+_i\hat{S}^-_j
                    + \hat{S}^-_i\hat{S}^+_j\right),
  \label{eq:4-heisenbergflipflop}
\end{equation}
which is how one implements it.  In the basis of $\bket{\uparrow\downarrow
\uparrow\cdots}$ configurations the first term is diagonal, contributing
$\pm J/4$ per bond according to whether the two spins are parallel or not,
and the second -- the flip-flop term -- exchanges an antiparallel pair.  It is
worth pausing on what is \emph{absent} here.  Spin operators on different
sites commute, so unlike every fermionic model in this chapter there is no
Jordan-Wigner string and no sign to track: the matrix elements of
Eq.~(\ref{eq:4-heisenbergflipflop}) are $+J/2$ and nothing else.  That single
fact is why spin models are the natural first target for quantum simulation.

\paragraph{Symmetries.}
The flip-flop term moves one unit of $S^z$ from one site to another, so the
total magnetisation $\hat{S}^z_{\mathrm{tot}}=\sum_i\hat{S}^z_i$ commutes with
$\hat{H}$ and the matrix is block diagonal in it; the ground state of the
antiferromagnet lies in the $S^z_{\mathrm{tot}}=0$ sector, of dimension
$\binom{N}{N/2}$.  The full $SU(2)$ is a symmetry as well, $[\hat{H},\bm{S}^2
_{\mathrm{tot}}]=0$, which is the same structure met in the Lipkin model of
Section~\ref{sec:lipkin} and in the $S$, $S_z$ classification of the pairing
model.  On a ring, translations give a further decomposition into momentum
sectors, and on a bipartite lattice the Marshall sign rule fixes the sign of
every amplitude in the ground state, which is what makes the
antiferromagnetic Heisenberg model free of the sign problem in quantum
Monte Carlo.

\paragraph{Checking the mapping from the Hubbard model.}
Section~\ref{sec:hubbard} predicted that the half-filled Hubbard ring at
strong coupling becomes the Heisenberg antiferromagnet with $J=4t^2/U$, and
that the four-site ring should approach $-3J$: $-2J$ from the exchange and
$-J$ from the constant $-\tfrac14\hat{n}_i\hat{n}_j$ term on four bonds.  We
can now verify both halves separately.  Diagonalising
Eq.~(\ref{eq:4-heisenbergH}) directly on the four-site ring gives
\begin{equation}
  E_0 = -2J,
  \qquad
  \langle\bm{S}_i\cdot\bm{S}_{i+1}\rangle = -\tfrac12,
  \qquad
  \langle\bm{S}_i\cdot\bm{S}_{i+2}\rangle = +\tfrac14,
  \label{eq:4-heisenberg4ring}
\end{equation}
the four nearest-neighbour bonds accounting for the whole energy.  And the
Hubbard energies of Table~\ref{tab:hubbardstrong}, with the constant $-J$
removed, converge on that number:
\begin{center}
\renewcommand{\arraystretch}{1.2}
\setlength{\tabcolsep}{12pt}
\begin{tabular}{rrr}
\toprule
$U/t$ & $E_0^{\mathrm{Hubbard}}/J$ & $E_0^{\mathrm{Hubbard}}/J+1$\\
\midrule
$8$   & $-2.640470$ & $-1.640470$\\
...
\end{tabular}
\end{center}
The last column tends to $-2$, which is Eq.~(\ref{eq:4-heisenberg4ring}).  The
two models really are the same at low energy, and we have now checked the
statement from both sides.

\paragraph{The Bethe ansatz and finite-size scaling.}
The one-dimensional spin-$\tfrac12$ chain is one of the very few interacting
many-body systems solved exactly in the thermodynamic limit.  Bethe's ansatz
gives the ground-state energy per site of the infinite antiferromagnetic
chain as
\begin{equation}
  \frac{E_0}{NJ} \;\longrightarrow\; \frac14 - \ln 2 = -0.4431471806\ldots,
  \label{eq:4-bethe}
\end{equation}
a number that no perturbative expansion in $J$ produces, since $J$ is the only
scale in the problem.  Table~\ref{tab:heisenberg} compares it with exact
diagonalisation of finite rings.  The convergence is instructive: the finite
chains lie \emph{below} the Bethe value and the deviation falls off as
$1/N^2$, so that even sixteen sites -- a matrix of dimension $12\,870$ in the
$S^z_{\mathrm{tot}}=0$ sector -- is still $0.7\%$ away.  Extracting the
thermodynamic limit from small systems is hard, and this is the cheapest
illustration of why.
...
\midrule
$\infty$ & --- & --- & $-0.44314718$ & $\phantom{-}0$\\
\bottomrule
\end{tabular}
\caption{The spin-$\tfrac12$ antiferromagnetic Heisenberg ring, $J=1$, $h=0$,
diagonalised in the $S^z_{\mathrm{tot}}=0$ sector.  The last row is the
Bethe-ansatz result~(\ref{eq:4-bethe}) and the last column is
$E_0/(NJ)-(\tfrac14-\ln2)$.  Beyond twelve sites the matrix is stored sparsely
and the lowest eigenvalue found by the Lanczos algorithm of
Section~\ref{sec:lanczos}.}
\label{tab:heisenberg}
\end{table}

\paragraph{Back to fermions.}
Equation~(\ref{eq:4-heisenbergH}) is not of the
form~(\ref{eq:4-generalH}), but it can be put there.  The Jordan-Wigner
transformation writes a spin-$\tfrac12$ chain as spinless fermions,
\begin{equation}
  \hat{S}^+_j = c^{\dagger}_j\prod_{k<j}\left(1-2\hat{n}_k\right),
  \qquad
  \hat{S}^z_j = \hat{n}_j - \tfrac12,
  \label{eq:4-jordanwigner}
\end{equation}
read backwards.  The string of $(1-2\hat{n}_k)=(-1)^{\hat{n}_k}$ is the
parity string~(\ref{eq:3-paritystring}) of
Section~\ref{sec:jordanwigner}, supplying exactly the sign that the
bit-counting algorithm of Section~\ref{sec:bitrep} computes.  On a
chain with nearest-neighbour couplings the strings cancel between the two
sites of a bond, and Eq.~(\ref{eq:4-heisenbergH}) becomes
\begin{equation}
  \hat{H} = \frac{J}{2}\sum_j
      \left(c^{\dagger}_jc_{j+1}+\mathrm{h.c.}\right)
    + J\sum_j
      \left(\hat{n}_j-\tfrac12\right)\left(\hat{n}_{j+1}-\tfrac12\right),
  \label{eq:4-heisenbergfermions}
\end{equation}
a hopping term plus a nearest-neighbour density-density interaction: the
Hubbard model of the previous section with the on-site $U$ replaced by an
inter-site one.  Dropping the $\hat{S}^z\hat{S}^z$ piece leaves the $XY$
model, which is free fermions and diagonal in momentum space.  Everything in
this book applies to Eq.~(\ref{eq:4-heisenbergfermions}) without
modification.

\begin{notebox}
\textbf{Why this model matters.} The Heisenberg model is the standard model of
quantum magnetism, and the reference system for essentially every numerical
method invented for correlated systems -- the density-matrix renormalisation
group was developed on it, and it is the benchmark for tensor networks,
quantum Monte Carlo and neural-network wave functions alike.  It is also the
model closest to a quantum computer: written in Pauli matrices,
Eq.~(\ref{eq:4-heisenbergH}) is already a sum of tensor products of
one- and two-qubit operators, with none of the Jordan-Wigner overhead that
fermionic Hamiltonians incur, so the Trotter circuits of
Chapter~\ref{chap:hartrefock} and the variational quantum eigensolver apply to
it directly.
\end{notebox}
...
The models above are all solvable because a symmetry cuts the Hilbert space
...
%----------------------------------------------------------------------
\section{The models on a quantum computer}
\label{sec:qubitmodels}

\index{Jordan-Wigner transformation!applied to the models}
\index{Pauli strings}
The models of this chapter are small, exactly solvable and highly structured,
which is precisely why they are the ones people run on quantum hardware.  This
section carries them across.  The tool is the Jordan-Wigner transformation of
Section~\ref{sec:jordanwigner}; the goal is the Pauli
form~(\ref{eq:3-paulisum}), $\hat{H}=\sum_{\alpha}h_{\alpha}P_{\alpha}$; and
the lesson -- which is the same lesson as the seniority -- is that what one
gets out depends on how much symmetry one is willing to exploit before
starting.

\paragraph{The general two-body Hamiltonian.}
Everything follows from four identities established in
Section~\ref{sec:jordanwigner}.  A diagonal one-body term becomes a sum of
$I$ and $Z$ by Eq.~(\ref{eq:3-jwnumber}); an off-diagonal one-body term
becomes the two strings of Eq.~(\ref{eq:3-jwhopping}); a pair-creation term
becomes the two strings of Eq.~(\ref{eq:3-jwpairtransfer}), differing from
the previous case only in one sign; and a two-body term with four distinct
indices becomes the eight weight-four strings of Eq.~(\ref{eq:3-jwtwobody}).
The general Hamiltonian~(\ref{eq:4-generalH}) is a sum of such terms, so its
qubit image is a sum of $\bigO(M^4)$ Pauli strings on $M$ qubits, one qubit per
spin-orbital.  Table~\ref{tab:3-paulicount} gave the counts.

That is the generic situation, and it is not encouraging: the parity strings
reach across the whole register, so the longest Pauli strings have weight $M$.
Every one of the models below does better than generic, and each does so for a
reason we have already met.

\paragraph{The pairing model, one qubit per spin-orbital.}
Order the spin-orbitals so that the two partners of a level are adjacent,
$(1+,1-,2+,2-,\dots)$, which keeps the parity strings inside a level empty.
With $L$ levels this needs $2L$ qubits.  The one-body part and the diagonal
$p=q$ part of the interaction are then products of $I$ and $Z$ alone,
\begin{equation}
  \hat{H}_0 + \hat{V}_{\mathrm{diag}}
   = \sum_{p}\left\{
     \xi(p-1)\left[\left(I-Z_{p+}\right)+\left(I-Z_{p-}\right)\right]
     - \frac{g}{8}\left(I-Z_{p+}\right)\left(I-Z_{p-}\right)
     \right\},
  \label{eq:4-jwpairingdiag}
\end{equation}
using $\hat{n}_{p\sigma}=(I-Z_{p\sigma})/2$ twice and
$\hat{P}^{+}_p\hat{P}^{-}_p=\hat{n}_{p+}\hat{n}_{p-}$.  The pair-transfer
terms with $p\ne q$ are the eight-string operators of
Eq.~(\ref{eq:3-jwtwobody}), one set for each pair of levels.  For $L=2$,
$\xi=1$ and $g=1$ the whole Hamiltonian is fifteen Pauli strings on four
qubits,
\begin{align}
  \hat{H}
   =\;& \tfrac{3}{4}\,IIII
     - \tfrac{3}{8}\left(IIZI+IIIZ\right)
     + \tfrac18\left(ZIII+IZII\right)
     - \tfrac18\left(ZZII+IIZZ\right)\nonumber\\
     & - \tfrac{1}{16}\Big(
         XXXX + XYXY + XYYX + YXXY + YXYX + YYYY\nonumber\\
     & \qquad\qquad - XXYY - YYXX
       \Big),
  \label{eq:4-jwpairingfull}
\end{align}
where the qubits are ordered $(1+,1-,2+,2-)$ and $XYXY$ means
$X_{1+}Y_{1-}X_{2+}Y_{2-}$.  Seven diagonal strings, eight of weight four.
The program \texttt{jordanwigner.py} produces this decomposition by taking
traces against all $4^4=256$ Pauli strings, so it is exact and not a
hand calculation to be trusted.

\paragraph{The pairing model, one qubit per level.}
We can do much better, and the reason is the seniority.  In the
seniority-zero space of Section~\ref{sec:pairingmodel} a level is either
empty or holds a complete pair, so its state is one bit, not two.  Make that
precise with the local quasispin operators
\begin{equation}
  \hat{S}^{+}_p = \hat{P}^{+}_p = a^{\dagger}_{p+}a^{\dagger}_{p-},
  \qquad
  \hat{S}^{-}_p = \hat{P}^{-}_p = a_{p-}a_{p+},
  \qquad
  \hat{S}^{z}_p = \tfrac12\left(\hat{N}_p-1\right),
  \label{eq:4-quasispinpair}
\end{equation}
with $\hat{N}_p=\hat{n}_{p+}+\hat{n}_{p-}$ the number of particles in level
$p$.  Exercise~\ref{ex:4-quasispin} shows from the anticommutation relations
alone that these obey
\begin{equation}
  \left[\hat{S}^{z}_p,\hat{S}^{\pm}_p\right] = \pm\hat{S}^{\pm}_p,
  \qquad
  \left[\hat{S}^{+}_p,\hat{S}^{-}_p\right] = 2\hat{S}^{z}_p,
  \qquad
  \left[\hat{S}^{\alpha}_p,\hat{S}^{\beta}_q\right] = 0
  \quad (p\ne q),
  \label{eq:4-quasispinalgebra}
\end{equation}
so each level carries its own $su(2)$ and the levels are independent.  This is
the same algebra that organised the Lipkin model in
Section~\ref{sec:lipkin}, and it is why the two models look so alike on a
quantum computer.

A spin one-half is a qubit.  Identifying $\bket{0}_p$ with an empty level and
$\bket{1}_p$ with an occupied one,
\begin{equation}
  \hat{S}^{+}_p = \frac{X_p-\imath Y_p}{2},
  \qquad
  \hat{S}^{-}_p = \frac{X_p+\imath Y_p}{2},
  \qquad
  \hat{S}^{z}_p = -\frac{Z_p}{2},
  \qquad
  \hat{N}_p = I-Z_p ,
  \label{eq:4-pairqubit}
\end{equation}
and the pairing Hamiltonian~(\ref{eq:4-pairingpairform}) collapses to
\begin{equation}
  \boxed{\;
  \hat{H}
   = \sum_{p=1}^{L}\left[\xi(p-1)-\frac{g}{4}\right]
       \left(I-Z_p\right)
     - \frac{g}{4}\sum_{p<q}\left(X_pX_q+Y_pY_q\right) \;}
  \label{eq:4-jwpairingcompact}
\end{equation}
on $L$ qubits.  The derivation is two lines: the $p=q$ part of the
interaction is $-\tfrac{g}{2}\sum_p\hat{S}^{+}_p\hat{S}^{-}_p
=-\tfrac{g}{4}\sum_p(I-Z_p)$, which merges with the one-body term, and the
$p\ne q$ part uses
\begin{equation}
  \hat{S}^{+}_p\hat{S}^{-}_q + \hat{S}^{+}_q\hat{S}^{-}_p
   = \frac12\left(X_pX_q+Y_pY_q\right).
  \label{eq:4-xyexchange}
\end{equation}
For two levels with $\xi=1$ and $g=1$ this is five strings on two qubits,
\begin{equation}
  \hat{H}^{(L=2)}
   = \tfrac12\,II + \tfrac14\,ZI - \tfrac34\,IZ
     - \tfrac14\left(XX+YY\right),
  \label{eq:4-jwpairingtwolevels}
\end{equation}
against fifteen strings on four qubits in
Eq.~(\ref{eq:4-jwpairingfull}).  Equation~(\ref{eq:4-xyexchange}) also
identifies what kind of spin model the pairing Hamiltonian is: an $XY$ model
with infinite-range couplings, in an inhomogeneous field.  Compare
Eq.~(\ref{eq:4-heisenbergH}) -- the $XY$ model is the Heisenberg model with
the $\hat{S}^z\hat{S}^z$ term removed, so the pairing model and the
antiferromagnet are close relatives once both are on qubits.

The compact form is a \emph{restriction}, not an approximation.  Its spectrum
is exactly the seniority-zero part of the full spectrum at fixed particle
number, level by level, and \texttt{jordanwigner.py} verifies this for $L=2$
and $L=3$ and every pair number.  Nothing is lost as long as the physics stays
in the seniority-zero space.

\paragraph{What the particle-hole term costs.}
And that is exactly what the particle-hole term of
Section~\ref{sec:pairingph} destroys.  Once $f\ne0$ the seniority-zero space
is no longer invariant -- Figure~\ref{fig:4-levels}(c) and~(d) are the states
that leak out -- so the one-qubit-per-level encoding is simply not available.
One has to go back to $2L$ qubits.  The leakage is easy to quantify: the
largest matrix element connecting the seniority-zero block to the rest is
$f/2$, exactly the $1p$--$1h$ matrix element of
Table~\ref{tab:pairingphcoupling}, and it is zero if and only if $f=0$.

The bill is presented in Table~\ref{tab:4-paulicounts}.  Adding the
particle-hole term to $L=3$ takes the Hamiltonian from $34$ Pauli strings of
weight at most four to $118$ strings of weight up to six, on the same six
qubits, while the compact encoding that would have needed three qubits and ten
strings is no longer legitimate at all.  Every one of those numbers is a
direct cost on hardware: strings are measurements in a variational
calculation, and weights are entangling gates in a Trotter step.
...
\begin{tabular}{llrrr}
\toprule
\textbf{Hamiltonian} & \textbf{Encoding} & \textbf{Qubits} &
\textbf{Pauli strings} & \textbf{Largest weight}\\
\midrule
...
\bottomrule
\end{tabular}
\caption{The cost of putting the pairing model on a quantum computer.  The
compact encoding halves the number of qubits and caps the Pauli weight at two,
but it requires the seniority to be conserved.  The particle-hole term breaks
that conservation, so the middle option becomes the only option and the count
grows sharply.  Produced by \texttt{jordanwigner.py}.}
\label{tab:4-paulicounts}
\end{table}

\paragraph{The other models.}
The same reading applies to the rest of the chapter.  The Lipkin model of
Section~\ref{sec:lipkin} is already written in quasispin operators, so it maps
onto a small number of qubits directly, and the compressed encoding that uses
$\lceil\log_2(2J+1)\rceil$ qubits for one $J$ block is the analogue of the
pair-qubit trick here.  The Hubbard model of Section~\ref{sec:hubbard} needs
one qubit per site and spin, and its hopping terms are the two strings of
Eq.~(\ref{eq:3-jwhopping}), which is why orbital ordering matters so much for
lattice models: a hop between distant sites in the chosen ordering drags a
long parity chain behind it.  The Heisenberg model of
Section~\ref{sec:heisenberg} is the easiest of all, because
Eq.~(\ref{eq:4-heisenbergH}) is \emph{already} a sum of Pauli strings of
weight two -- there is no transformation to perform, and no parity strings to
pay for.  That is the point made in passing in
Section~\ref{sec:heisenberg}, now with the cost of the alternative in view.

\begin{notebox}
\textbf{Quantum-computing connection.} All of this is preparation for the
variational quantum eigensolver.  There one prepares a parametrised state
$\bket{\Psi(\theta)}$ on the register, measures
$\bracket{\Psi(\theta)}{\hat{H}}{\Psi(\theta)}$ by measuring each Pauli string
of the decomposition separately, and minimises over $\theta$ classically.  The
unitary coupled-cluster ansatz of Section~\ref{sec:ucc} supplies the state,
the Trotter expansion of Section~\ref{sec:trotter} turns it into a circuit,
and Eq.~(\ref{eq:4-jwpairingcompact}) supplies the observable.  The pairing
model with $L=2$ is then a genuine two-qubit calculation which can be run on
hardware; with the particle-hole term switched on it is a four-qubit
calculation, which is a rather different proposition.
\end{notebox}

%----------------------------------------------------------------------
\section{What the models have in common}
\label{sec:modelssummary}

Table~\ref{tab:models} collects them.  The pattern is worth stating
...
\begin{tabular}{lllll}
\toprule
\textbf{Model} & \textbf{One-body space} & \textbf{Interaction} &
\textbf{Symmetry used} & \textbf{Exactly solvable}\\
\midrule
Lipkin   & two levels, degeneracy $\Omega$ & pair transfer, exchange
         & $SU(2)$ quasispin & yes, block by block\\
Pairing  & $L$ doubly degenerate levels & pair transfer
         & $S$, $S_z$, seniority & yes, small $L$\\
Pairing $+$ p-h & $L$ doubly degenerate levels & $+$ pair breaking
         & $S_z$ only & yes, small $L$\\
Hubbard  & lattice sites & on-site $U$
         & $N_{\uparrow}$, $N_{\downarrow}$, momentum & 1D only\\
Heisenberg & spins on a lattice & exchange $\bm{S}_i\cdot\bm{S}_j$
         & $S^z_{\mathrm{tot}}$, $SU(2)$ & 1D only\\
Calogero & continuum, $N$ particles & inverse square
         & integrability & yes, any $N$\\
\bottomrule
\end{tabular}
\caption{The models of this chapter.  Each is the general
Hamiltonian~(\ref{eq:4-generalH}) with a drastically restricted set of
two-body matrix elements -- the Heisenberg model after the Jordan-Wigner
transformation~(\ref{eq:4-jordanwigner}) -- and in each case a symmetry
reduces the dimension of the problem.  The third row is the second row with
one term added, and the price of that term is visible in the fourth column:
the seniority is gone, and with it the reduction from thirty-six dimensions
to six.}
\label{tab:models}
\end{table}
...
\subsection*{Exercise 5: the particle-hole term}
\label{ex:4-pairingph}
\addcontentsline{toc}{subsection}{Exercise 5: the particle-hole term}

\begin{enumerate}
  \item[a)] Show that $\hat{V}_{\mathrm{ph}}$ of Eq.~(\ref{eq:4-Vph})
  conserves $N_+$ and $N_-$ separately, and hence that $S_z$ remains a good
  quantum number.
  \item[b)] Isolate the $q=r$ part of the sum and show that, together with its
  Hermitian conjugate, it is the pairing interaction with $g$ replaced by
  $2f$.  Deduce the diagonal matrix element of $\hat{H}$ in the reference
  determinant of Figure~\ref{fig:4-levels}(a) for $L=4$, $N=4$.
  \item[c)] Let $\bket{c}$ be that reference determinant and let
  $\bket{\Phi_1}$ be the one-particle-one-hole determinant of
  Figure~\ref{fig:4-levels}(c).  Show that
  $\bracket{\Phi_1}{\hat{V}_{\mathrm{pair}}}{c}=0$ and evaluate
  $\bracket{\Phi_1}{\hat{V}_{\mathrm{ph}}}{c}$.
  \item[d)] Show that the seniority is not conserved once $f\ne0$, by
  exhibiting a state of seniority two that $\hat{H}$ reaches from $\bket{c}$.
\end{enumerate}

\paragraph{Answer to a).}
Every term of $\hat{V}_{\mathrm{ph}}$ contains
$a^{\dagger}_{p+}a^{\dagger}_{p-}a_{q-}a_{r+}$: one spin-up operator is
created and one destroyed, and likewise for spin down.  So
$[\hat{V}_{\mathrm{ph}},\hat{N}_+]=[\hat{V}_{\mathrm{ph}},\hat{N}_-]=0$, and
since $S_z=(\hat{N}_+-\hat{N}_-)/2$ it commutes with $S_z$ as well.  Note that
this is weaker than the pairing case: there $\hat{V}_{\mathrm{pair}}$ commutes
with $\hat{S}^2$ too, and with the seniority operator; here neither holds.

\paragraph{Answer to b).}
Setting $q=r$ in Eq.~(\ref{eq:4-Vph}) gives
\[
  -\frac{f}{2}\sum_{pq} a^{\dagger}_{p+}a^{\dagger}_{p-}a_{q-}a_{q+} ,
\]
which is $\hat{V}_{\mathrm{pair}}$ with $g\to f$.  Its Hermitian conjugate is
$-\tfrac{f}{2}\sum_{pq}a^{\dagger}_{q+}a^{\dagger}_{q-}a_{p-}a_{p+}$, and
relabelling $p\leftrightarrow q$ shows it to be the same sum again.  The two
together therefore contribute $\hat{V}_{\mathrm{pair}}$ with $g\to2f$, so in
every seniority-conserving matrix element $g$ is replaced by $g+2f$.  For
$L=4$, $N=4$ the reference has complete pairs in levels $1$ and $2$, so
\[
  \bracket{c}{\hat{H}}{c}
   = \underbrace{2\xi(0)+2\xi(1)}_{\hat{H}_0}
     \;-\;\frac{g+2f}{2}\times 2
   = 2\xi - (g+2f),
\]
which is $0.9$ for $\xi=1$, $g=1$, $f=0.05$ -- the first entry of
Table~\ref{tab:pairingph}.

\paragraph{Answer to c).}
$\bket{\Phi_1}$ differs from $\bket{c}$ in a single spin-orbital: the
spin-up particle of level $2$ has moved to level $3$.  A two-body operator can
connect determinants differing by one orbital only through its
``$\hat{n}$-contracted'' pieces, and $\hat{V}_{\mathrm{pair}}$ has none: every
term $a^{\dagger}_{p+}a^{\dagger}_{p-}a_{q-}a_{q+}$ changes the occupation of
level $q$ by two and of level $p$ by two, so the two determinants must differ
by an even number of orbitals in \emph{each} spin channel.  Hence
$\bracket{\Phi_1}{\hat{V}_{\mathrm{pair}}}{c}=0$, and the particle-hole
channel of the pure pairing model is empty.

For $\hat{V}_{\mathrm{ph}}$ take the Hermitian-conjugate term with
$r=3$, $p=q=2$,
\[
  -\frac{f}{2}\,a^{\dagger}_{3+}a^{\dagger}_{2-}a_{2-}a_{2+}
   = -\frac{f}{2}\,a^{\dagger}_{3+}\hat{n}_{2-}a_{2+},
\]
which acting on $\bket{c}$ gives exactly $\bket{\Phi_1}$, since level $2$ is
full.  Collecting the signs from the anticommutations,
$\bracket{\Phi_1}{\hat{V}_{\mathrm{ph}}}{c}=\pm f/2$, the sign depending on
the ordering convention for the spin-orbitals.  This is the entry $\pm f/2$ of
Table~\ref{tab:pairingphcoupling}, and \texttt{models.py} confirms that all
eight one-particle-one-hole determinants have matrix elements of this
magnitude.

\paragraph{Answer to d).}
Take $p=3$, $q=2$, $r=1$ in Eq.~(\ref{eq:4-Vph}):
$a^{\dagger}_{3+}a^{\dagger}_{3-}a_{2-}a_{1+}$ removes the spin-down particle
from level $2$ and the spin-up particle from level $1$ and deposits a complete
pair in level $3$.  The result is Figure~\ref{fig:4-levels}(d): levels $1$ and
$2$ each retain a single, unpaired particle, so the seniority is two, while
$\bket{c}$ had seniority zero.  A single application of $\hat{H}$ therefore
changes the seniority, and the seniority-zero space of
Section~\ref{sec:pairingmodel} is no longer invariant.

\subsection*{Exercise 6: the Heisenberg model}
\label{ex:4-heisenberg}
\addcontentsline{toc}{subsection}{Exercise 6: the Heisenberg model}

\begin{enumerate}
  \item[a)] Verify Eq.~(\ref{eq:4-heisenbergflipflop}) from the definitions
  $\hat{S}^{\pm}=\hat{S}^x\pm i\hat{S}^y$, and show that
  $[\hat{H},\hat{S}^z_{\mathrm{tot}}]=0$.
  \item[b)] Solve the two-site problem exactly, and identify the singlet and
  the triplet.
  \item[c)] For the four-site ring, use the value $E_0=-2J$ quoted in
  Eq.~(\ref{eq:4-heisenberg4ring}) together with translational invariance to
  deduce $\langle\bm{S}_i\cdot\bm{S}_{i+1}\rangle$, and then obtain
  $\langle\bm{S}_i\cdot\bm{S}_{i+2}\rangle$ from the fact that the ground
  state is a total spin singlet.
  \item[d)] Verify the Jordan-Wigner
  transformation~(\ref{eq:4-jordanwigner}): show that the strings cancel for a
  nearest-neighbour bond and that Eq.~(\ref{eq:4-heisenbergfermions}) follows.
\end{enumerate}

\paragraph{Answer to a).}
$\hat{S}^+_i\hat{S}^-_j+\hat{S}^-_i\hat{S}^+_j
 = 2(\hat{S}^x_i\hat{S}^x_j+\hat{S}^y_i\hat{S}^y_j)$, since the cross terms
$i(\hat{S}^y_i\hat{S}^x_j-\hat{S}^x_i\hat{S}^y_j)$ cancel between the two
products.  Adding $\hat{S}^z_i\hat{S}^z_j$ gives
Eq.~(\ref{eq:4-heisenbergflipflop}).  For the commutator, note that
$\hat{S}^z_i\hat{S}^z_j$ is diagonal and that $\hat{S}^+_i\hat{S}^-_j$ raises
$S^z$ on site $i$ and lowers it on site $j$, leaving the sum unchanged.

\paragraph{Answer to b).}
With one bond, $\hat{H}=J\bm{S}_1\cdot\bm{S}_2
 = \tfrac{J}{2}\left(\bm{S}^2_{\mathrm{tot}}
   -\bm{S}^2_1-\bm{S}^2_2\right)
 = \tfrac{J}{2}\left(S(S+1)-\tfrac32\right)$.
The singlet $S=0$ has $E=-3J/4$ and the triplet $S=1$ has $E=+J/4$: the
familiar splitting, and the reason a bond wants to be a singlet.  The gap is
$J$.

\paragraph{Answer to c).}
Translational invariance makes all four nearest-neighbour bonds equivalent, so
$E_0=4J\langle\bm{S}_i\cdot\bm{S}_{i+1}\rangle=-2J$ gives
$\langle\bm{S}_i\cdot\bm{S}_{i+1}\rangle=-\tfrac12$.  For the second, use
\[
  0 = \langle\bm{S}^2_{\mathrm{tot}}\rangle
    = \sum_{ij}\langle\bm{S}_i\cdot\bm{S}_j\rangle
    = 4\cdot\tfrac34 + 8\langle\bm{S}_i\cdot\bm{S}_{i+1}\rangle
      + 4\langle\bm{S}_i\cdot\bm{S}_{i+2}\rangle ,
\]
where the singlet ground state has $\bm{S}^2_{\mathrm{tot}}=0$, each site
contributes $\bm{S}_i^2=\tfrac34$, there are eight ordered nearest-neighbour
pairs and four ordered next-nearest ones.  Solving,
$\langle\bm{S}_i\cdot\bm{S}_{i+2}\rangle
 = -\tfrac14(3-4)=+\tfrac14$.  Both values are confirmed by
\texttt{models.py}.  The positive next-nearest-neighbour correlation is the
antiferromagnet: sites two apart are on the same sublattice.

\paragraph{Answer to d).}
Write $K_j=\prod_{k<j}(1-2\hat{n}_k)$, so that
$\hat{S}^+_j=c^{\dagger}_jK_j$ and $\hat{S}^-_j=K_jc_j$.  For $j$ and $j+1$,
$K_jK_{j+1}=(1-2\hat{n}_j)$, since every factor with $k<j$ appears twice and
$(1-2\hat{n}_k)^2=1$.  Then
\[
  \hat{S}^+_j\hat{S}^-_{j+1}
   = c^{\dagger}_j(1-2\hat{n}_j)c_{j+1}
   = c^{\dagger}_jc_{j+1},
\]
because $c^{\dagger}_j$ acts only on states with $\hat{n}_j=0$, where the
factor is unity.  The strings have cancelled.  With
$\hat{S}^z_j=\hat{n}_j-\tfrac12$ the diagonal term is
$J(\hat{n}_j-\tfrac12)(\hat{n}_{j+1}-\tfrac12)$, and collecting both gives
Eq.~(\ref{eq:4-heisenbergfermions}).  The function
\texttt{jordan\_wigner\_chain} in \texttt{models.py} builds the fermionic form
directly and confirms that the two Hamiltonians have identical spectra, not
merely identical ground-state energies, for open chains of four to ten sites.

\subsection*{Exercise 7: quasispin and the qubit encodings}
\label{ex:4-quasispin}
\addcontentsline{toc}{subsection}{Exercise 7: quasispin and qubit encodings}

\begin{enumerate}
  \item[a)] Derive the quasispin algebra~(\ref{eq:4-quasispinalgebra}) from
  the fermion anticommutation relations alone.
  \item[b)] Verify Eq.~(\ref{eq:4-xyexchange}) and deduce the compact
  form~(\ref{eq:4-jwpairingcompact}).
  \item[c)] Write out the one-level pairing Hamiltonian
  $\hat{H}=\varepsilon_p\hat{N}_p-g\,\hat{n}_{p+}\hat{n}_{p-}$ in the full
  two-qubit space and show that it is diagonal.
  \item[d)] Explain, without computing anything, why the compact encoding
  cannot be used once $f\ne0$.
\end{enumerate}

\paragraph{Answer to a).}
For the first commutator use $[\hat{N}_p,a^{\dagger}_{p\sigma}]
=a^{\dagger}_{p\sigma}$, which follows from
$\{a_{p\sigma},a^{\dagger}_{p\sigma}\}=1$.  Applying it twice,
$[\hat{N}_p,a^{\dagger}_{p+}a^{\dagger}_{p-}]=2a^{\dagger}_{p+}
a^{\dagger}_{p-}$, so
\[
  \left[\hat{S}^{z}_p,\hat{S}^{+}_p\right]
   = \tfrac12\left[\hat{N}_p,\hat{S}^{+}_p\right]
   = \hat{S}^{+}_p ,
\]
and the Hermitian conjugate gives the $-$ case.  For the second, expand
\[
  \hat{S}^{+}_p\hat{S}^{-}_p
   = a^{\dagger}_{p+}a^{\dagger}_{p-}a_{p-}a_{p+}
   = a^{\dagger}_{p+}\hat{n}_{p-}a_{p+}
   = \hat{n}_{p+}\hat{n}_{p-},
\]
and, using $a_{p+}a^{\dagger}_{p+}=1-\hat{n}_{p+}$ twice,
\[
  \hat{S}^{-}_p\hat{S}^{+}_p
   = a_{p-}a_{p+}a^{\dagger}_{p+}a^{\dagger}_{p-}
   = \left(1-\hat{n}_{p+}\right)\left(1-\hat{n}_{p-}\right).
\]
Subtracting,
$[\hat{S}^{+}_p,\hat{S}^{-}_p]=\hat{n}_{p+}+\hat{n}_{p-}-1
=\hat{N}_p-1=2\hat{S}^{z}_p$.  Operators belonging to different levels
involve disjoint sets of orbitals and each contains an even number of fermion
operators, so they commute.

\paragraph{Answer to b).}
With $\hat{S}^{\pm}=(X\mp\imath Y)/2$ on each qubit,
\[
  \hat{S}^{+}_p\hat{S}^{-}_q
   = \tfrac14\left(X_pX_q+Y_pY_q
     +\imath X_pY_q-\imath Y_pX_q\right),
\]
and adding the term with $p$ and $q$ interchanged cancels the imaginary
parts, leaving $\tfrac12(X_pX_q+Y_pY_q)$.  Hence
$-\tfrac{g}{2}\sum_{p\ne q}\hat{S}^{+}_p\hat{S}^{-}_q
=-\tfrac{g}{4}\sum_{p<q}(X_pX_q+Y_pY_q)$.  The diagonal $p=q$ terms give
$-\tfrac{g}{2}\sum_p\hat{n}_{p+}\hat{n}_{p-}
=-\tfrac{g}{4}\sum_p(I-Z_p)$ in the paired subspace, and the one-body term
contributes $\xi(p-1)$ per particle, that is $2\xi(p-1)$ per pair, or
$\xi(p-1)(I-Z_p)$.  Collecting gives
Eq.~(\ref{eq:4-jwpairingcompact}).

\paragraph{Answer to c).}
Both terms are built from number operators, and
$\hat{n}_{p\sigma}=(I-Z_{p\sigma})/2$, so
\[
  \hat{H}
   = \frac{\varepsilon_p}{2}\left(2I-Z_{p+}-Z_{p-}\right)
     - \frac{g}{4}\left(I-Z_{p+}\right)\left(I-Z_{p-}\right)
   = \left(\varepsilon_p-\frac{g}{4}\right)I
     - \left(\frac{\varepsilon_p}{2}-\frac{g}{4}\right)
       \left(Z_{p+}+Z_{p-}\right)
     - \frac{g}{4}Z_{p+}Z_{p-} .
\]
Only $I$ and $Z$ appear, so the Hamiltonian is diagonal in the computational
basis and its eigenvalues can be read off: $0$, $\varepsilon_p$,
$\varepsilon_p$ and $2\varepsilon_p-g$ for zero, one and two particles.  A
single pair level needs no quantum computer.

\paragraph{Answer to d).}
The compact encoding assigns one qubit to a whole level, which presupposes
that the level is either empty or fully occupied -- that is, that the
seniority is zero.  The particle-hole term produces the states of
Figure~\ref{fig:4-levels}(c) and~(d), in which a level carries a single
unpaired particle, and those states have no representative in the compact
Hilbert space.  Restricting to it would not be an approximation but a
projection onto a subspace the Hamiltonian does not leave invariant, and the
resulting energies would be wrong even qualitatively.

\subsection*{Exercise 8: numerical explorations}
\addcontentsline{toc}{subsection}{Exercise 8: numerical explorations}
