...
if __name__ == "__main__":
    ...
    print("="*80)
    print("PAIRING MODEL: EXACT vs MBPT2 vs CCD")
    print("="*80)
    ...
    print("COMPARISON")
    print("="*80)
    print(f"\n{'Method':15s} | {'Total E':>14s} | {'Corr E':>14s} | {'Error':>12s}")
    print("-"*70)
    print(f"{'Exact':15s} | {E_exact:+14.10f} | {E_exact-E_ref:+14.10f} | {'--':>12s}")
    print(f"{'MBPT2':15s} | {E_ref+E_mbpt2_corr:+14.10f} | {E_mbpt2_corr:+14.10f} | {abs(E_ref+E_mbpt2_corr-E_exact):12.2e}")
    print(f"{'CCD':15s} | {E_ref+E_ccd_corr:+14.10f} | {E_ccd_corr:+14.10f} | {abs(E_ref+E_ccd_corr-E_exact):12.2e}")
    ...
    print("STATISTICS")
    print("="*80)
    print(f"\n{'Metric':25s} | {'MBPT2':>15s} | {'CCD':>15s}")
    print("-"*65)
    print(f"{'Max error':25s} | {np.max(error_mbpt2):15.6e} | {np.max(error_ccd):15.6e}")
    print(f"{'Mean error':25s} | {np.mean(error_mbpt2):15.6e} | {np.mean(error_ccd):15.6e}")
    print(f"{'RMS error':25s} | {np.sqrt(np.mean(error_mbpt2**2)):15.6e} | {np.sqrt(np.mean(error_ccd**2)):15.6e}")
    improvement = np.mean(error_mbpt2) / np.mean(error_ccd)
    print(f"\nCCD is {improvement:.1f}× more accurate than MBPT2")
    ...
    # Plot 1: Energies
    ax1 = axes[0, 0]
    ax1.plot(g_values, energies_exact, 'k-', lw=2.5, label='Exact')
    ax1.plot(g_values, energies_ccd, 'b--', lw=2, label='CCD')
    ax1.plot(g_values, energies_mbpt2, 'r:', lw=2, label='MBPT2')
    ax1.set_xlabel('Pairing strength g', fontsize=12)
    ax1.set_ylabel('Energy', fontsize=12)
    ax1.set_title('Energy vs g', fontweight='bold', fontsize=13)
    ...
    ax2.set_title('Absolute Error', fontweight='bold', fontsize=13)
    ax2.legend(fontsize=11)
    ax2.grid(alpha=0.3, which='both')
    
    # Plot 3: Differences
    ax3 = axes[1, 0]
    ax3.plot(g_values, energies_exact - energies_mbpt2, 'r-', lw=2, label='Exact - MBPT2')
    ax3.plot(g_values, energies_exact - energies_ccd, 'b-', lw=2, label='Exact - CCD')
    ...
    ax3.set_title('Energy Differences', fontweight='bold', fontsize=13)
    ax3.legend(fontsize=11)
    ax3.grid(alpha=0.3)
    
    # Plot 4: Improvement factor
    ax4 = axes[1, 1]
    improvement_factor = error_mbpt2 / (error_ccd + 1e-12)
    ax4.plot(g_values, improvement_factor, 'g-', lw=2)
    ax4.axhline(1, color='k', ls='--', alpha=0.5)
    ax4.set_xlabel('Pairing strength g', fontsize=12)
    ax4.set_ylabel('Error(MBPT2) / Error(CCD)', fontsize=12)
    ax4.set_title('CCD Improvement Factor', fontweight='bold', fontsize=13)
    ax4.grid(alpha=0.3)
    
    plt.suptitle('Pairing Model: Exact vs MBPT2 vs CCD', fontsize=14, fontweight='bold')
    plt.tight_layout()
    plt.savefig('pairing_comparison.png', dpi=150, bbox_inches='tight')
    print("\n✓ Plot saved: pairing_comparison.png")
    plt.show()
